...
import { loadState, saveState, withStateLock } from '../lib/state';
...
function optionalInt(value: string | boolean | undefined): number | null {
...
export function run(args: string[]): void {
  ...
  if (worktreePath) {
    try {
      ...
      warnings.push(`git worktree remove 失败（可能已不在注册列表中）：${(err as Error).message}`);
    }
    ...
    if (existsSync(worktreePath)) {
      try {
        ...
        try {
          ...
          warnings.push(`git worktree prune 失败：${(pruneErr as Error).message}`);
        }
      } catch (rmErr) {
        warnings.push(`rmSync 兜底删除目录失败：${(rmErr as Error).message}`);
      }
    }
  }
  ...
  try {
    ...
    warnings.push(
      `本地 Phase 分支 "${branchName}" 未能安全删除（git branch -d 失败，可能本地 main 还未合并）。` +
  ...
  try {
    ...
    warnings.push(`进度文件更新失败（phaseWorktreePath 未置 null）：${(err as Error).message}`);
  }
