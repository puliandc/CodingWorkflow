...
import { parseArchContracts } from '../lib/state';
...
describe('parseArchContracts', () => {
  it('正常 arch.md → 解析出三表', () => {
    ...
    const result = parseArchContracts(md);

    // whitelist
    assert.ok(result.whitelist.includes('src/foo.ts'), `whitelist 应包含 src/foo.ts，实际=${JSON.stringify(result.whitelist)}`);
    assert.ok(result.whitelist.includes('src/bar.ts'), `whitelist 应包含 src/bar.ts`);

    // frozen
    assert.ok(result.frozen.includes('src/core.ts'), `frozen 应包含 src/core.ts，实际=${JSON.stringify(result.frozen)}`);

    // regressionGuards（护栏节在白名单前，保证正则可命中终止边界）
    assert.ok(result.regressionGuards.includes('parseArchContracts 正常解析'),
      `regressionGuards 应包含 'parseArchContracts 正常解析'，实际=${JSON.stringify(result.regressionGuards)}`);
  });

  it('白名单为空时 whitelist 数组为 []', () => {
    const md = makeArchMd({ whitelistRows: [] });
    const result = parseArchContracts(md);
    assert.deepEqual(result.whitelist, []);
  });

  it('护栏为空时 regressionGuards 数组为 []', () => {
    const md = makeArchMd({ guardRows: [] });
    const result = parseArchContracts(md);
    assert.deepEqual(result.regressionGuards, []);
  });

  it('缺少文件白名单表时 whitelist 为 []（无 header → 无法匹配）', () => {
    // 完全去掉白名单节
    const md = `# arch.md\n\n#### [契约] 冻结表\n\n| 冻结路径 | 说明 |\n| --- | --- |\n| \`a.ts\` | 说明 |\n`;
    const result = parseArchContracts(md);
    assert.deepEqual(result.whitelist, []);
  ...
  it('缺少所有三个契约表时返回全空结构', () => {
    const md = '# 架构文档\n\n无契约信息。\n';
    const result = parseArchContracts(md);
    assert.deepEqual(result, { whitelist: [], frozen: [], regressionGuards: [] });
  });
  ...
  it('行内含占位符"无"时被过滤', () => {
    ...
    const result = parseArchContracts(md);
    assert.ok(!result.regressionGuards.includes('无'), `"无"应被过滤，实际=${JSON.stringify(result.regressionGuards)}`);
  });
});
